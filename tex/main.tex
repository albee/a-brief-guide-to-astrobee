...
\item A lookup table is used to map $T_N$ to $N$, the impeller speed. Then, $\frac{\sum T_N}{N^2\times C_d}$ is mapped to $Cd_p$ using another lookup table.
$\Delta P$ or the normalized pressure density: 
\begin{equation}
\Delta P = Cd_p\times P \times N^2 \times d^2
\end{equation}
$d$: impeller diameter\\
$N$: impeller speed\\
...
$p$ : Air density\\
\begin{equation}
A_i = \frac{T_N}{2\times\Delta P \times C_d^2}
\end{equation}
...
\begin{equation}
cos(\theta) = \frac{H - S_i}{L}
\end{equation}
$H$: Nozzle height\\
$L$: Flap length\\
$S_i$: Commanded nozzle opening\\
$\theta_i$: Commanded nozzle opening angle. The maximum allowed nozzle opening is about 65$\degree$
\\$A_i$: Commanded area per nozzle For each nozzle $i$\\
\smallskip
After checking that it lies within the limits, i.e, if $\theta<\theta_{\textrm{min}}$, $\theta = \theta_{\textrm{min}}$ and if $\theta>\theta_{\textrm{max}}$, $\theta = \theta_{\textrm{max}}$, the commanded angles is sent off to pwm to convert to servo commands.\\
